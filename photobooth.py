...
class Photobooth:
    ...
    def __init__(self, display_size, picture_basename, picture_size, preview_size, pose_time, display_time,
                 ...
                 cam_list=['sony_wifi', 'picam', 'webcam', 'dslr', 'dummicam'], ssid="DIRECT-LKE0:ILCE-6000"):
        ...
        self.camera = camera.get_camera(picture_size, preview_size, priority_list=cam_list, ssid=ssid)
        # self.preview_camera=camera.get_camera(picture_size, preview_size,['picam', 'webcam', 'dslr','dummicam'], self.camera)
        # self.preview_camera=camera.get_camera(picture_size, preview_size, priority_list=cam_list, ssid=ssid,default_cam=self.camera)
    ...
    def get_info_text(self):
        ...
        return ("Camera: {}\n\n{}\n\nprinter: {}".format(self.camera_info(), self.pictures.get_info(),
                                                         self.print_queue.get_printer_state()))
...
class DisplayPage:
    ...
    def get_pos(self, field, dim=(2, 2), frame=(50, 50, 50, 50)):  # frame: TLBR
        ...
        return [round((field[i] + .5) * (self.display.size[i] - frame[0 + i] - frame[2 + i]) / dim[i] + frame[0 + 1])
                for i in range(2)]
...
class PhotobothPage(DisplayPage):
    def __init__(self, name, pb, options=None, timer=5, bg=None, overlay_text=None):
        self.pb = pb
        DisplayPage.__init__(self, name, pb.display, options=options, timer=timer, bg=bg, overlay_text=overlay_text,
                             teardown=pb.teardown)
...
class SlideshowPage(PhotobothPage):
    ...
    def apply(self):
        ...
        self.display.add_button(img_fn=self.bg, size=self.display.get_size(), adj=(1, 1), press_depth=1, action_value=1,
                                keep_ratio=True)
        self.display.show_message("#{}".format(self.photo_idx), font_size=self.overlay_text_size, adj=(0, 0))
        self.display.add_button(img_fn=self.pb.theme.get_file_name("left_button"), size=(50, 50), adj=(0, 1),
                                action_value=2)
        self.display.add_button(img_fn=self.pb.theme.get_file_name("right_button"), size=(50, 50), adj=(2, 1),
                                action_value=3)
        self.display.add_button(img_fn=self.pb.theme.get_file_name("picture_opt"), size=(50, 50), adj=(2, 2),
                                action_value=4)
...
class ResultPage(PhotobothPage):
    ...
    def apply(self):
        ...
        self.display.add_button(action_value=2, adj=(0, 2), img_fn=self.pb.theme.get_file_name("trashbin"),
                                size=(100, 100))
...
class SettingsPage(PhotobothPage):
    def __init__(self, pb):
        options = [pb.show_main, pb.show_main, pb.show_layout, pb.show_filter, self.zoom_out, self.zoom_in,
                   self.next_theme, self.prev_theme, self.del_printjobs, self.bubble_connect, self.bubble_down,
                   self.bubble_up,
    ...
    def apply(self):
        ...
        self.display.add_button(action_value=1, adj=(2, 2), img_fn=self.pb.theme.get_file_name("return"),
                                size=(100, 100))
        self.display.add_button(action_value=14, adj=(0, 2), img_fn=self.pb.theme.get_file_name("exit"),
                                size=(100, 50))
        self.display.add_button(action_value=2, pos=(cols[2], rows[0]), adj=(1, 1), size=[300, b_size],
                                img_fn=self.pb.theme.get_file_name("layout_options_small"))
        self.display.add_button(action_value=3, pos=(cols[4], rows[0]), adj=(1, 1), size=[300, b_size],
                                img_fn=self.pb.theme.get_file_name("filter_options_small"))
        self.display.show_message("Zoom:", font_size=50, pos=(cols[1], rows[1]), adj=(1, 1))
        self.display.show_message("{} mm".format(self.pb.camera.get_zoom()), font_size=50, pos=(cols[3], rows[1]),
                                  adj=(1, 1))
        self.display.add_button(action_value=4, pos=(cols[3] - 100, rows[1]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("left_button"))
        self.display.add_button(action_value=5, pos=(cols[3] + 100, rows[1]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("right_button"))
        ...
        self.display.add_button(action_value=6, pos=(cols[3] - 100, rows[2]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("left_button"))
        self.display.add_button(action_value=7, pos=(cols[3] + 100, rows[2]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("right_button"))
        self.display.show_message("Printer:", font_size=50, pos=(cols[1], rows[3]), adj=(1, 1))
        self.display.add_button(action_value=8, pos=(cols[2], rows[3]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("printer"))
        p_ready, p_msg = self.pb.print_queue.get_printer_state()
        ...
        self.display.add_button(action_value=9, pos=(cols[2], rows[4]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("ble"))
        if not self.pb.bubble_canon.is_supported():
            self.display.show_message("no ble support", font_size=50, pos=((cols[2] + cols[3]) / 2, rows[4]),
                                      adj=(2, 1))

        elif self.pb.bubble_canon.has_connection():
            self.display.add_button(action_value=10, pos=(cols[3] - 75, rows[4]), adj=(1, 1), size=[b_size, b_size],
                                    img_fn=self.pb.theme.get_file_name("left_button"))
            self.display.add_button(action_value=11, pos=(cols[3] + 75, rows[4]), adj=(1, 1), size=[b_size, b_size],
                                    img_fn=self.pb.theme.get_file_name("right_button"))
            self.display.show_message("{} %".format(self.pb.bubble_prob), font_size=50, pos=(cols[3], rows[4]),
                                      adj=(1, 1))
        ...
        self.display.add_button(action_value=12, pos=(cols[3] - 100, rows[5]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("left_button"))
        self.display.add_button(action_value=13, pos=(cols[3] + 100, rows[5]), adj=(1, 1), size=[b_size, b_size],
                                img_fn=self.pb.theme.get_file_name("right_button"))
    ...
    def bubble_connect(self):
        if self.pb.bubble_canon.is_supported():
            self.display.show_message("scanning...", font_size=50,
                                      pos=self.get_pos((3, 4), dim=(6, 6), frame=(0, 0, 0, 0)), adj=(1, 1))
            self.display.apply()
...
class LayoutPage(PhotobothPage):
    def __init__(self, pb):
        ...
        for i in range(N_LAYOUTOPT):
            ...
            self.example_img.append(layout.assemble_pictures(self.example_img_raw * layout.get_npic(), pb.theme,
    ...
    def apply(self):
        ...
        for i in range(N_LAYOUTOPT):
            ...
            self.display.add_button(action_value=i + 2, pos=self.get_pos(i, self.grid_dim), adj=(1, 1), img_fn=button,
                                    size=(100, 100), press_depth=1)
        self.display.add_button(action_value=1, adj=(2, 2), img_fn=self.pb.theme.get_file_name("return"),
                                size=(100, 100))
...
class TimePage(DisplayPage):
    ...
    def apply(self):
        ...
        for i in range(3):
            self.display.add_button(action_value=i * 2 + 2, pos=self.get_pos((i + 1, 1), self.grid_dim), adj=(1, 1),
                                    img_fn=self.theme.get_file_name("up_button"), size=(100, 100), press_depth=1)
            self.display.show_message(str(self.get(i)), pos=self.get_pos((i + 1, 2), self.grid_dim), adj=(1, 1))
            self.display.add_button(action_value=i * 2 + 3, pos=self.get_pos((i + 1, 3), self.grid_dim), adj=(1, 1),
                                    img_fn=self.theme.get_file_name("down_button"), size=(100, 100), press_depth=1)
        self.display.add_button(action_value=0, adj=(1, 2), text="OK", frame_color=(255, 0, 0),
                                font_color=(255, 255, 255), size=(100, 50))
        self.display.apply()
...
def main(args):
    ...
    photobooth = Photobooth(args.display_size, picture_basename, args.image_size, args.preview_size, args.pose_time,
                            args.display_time,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ds', '--display_size', metavar='WWWxHHH', help="the size of the window/screen",
                        default=(1024, 600), action=ResolutionAction)
    parser.add_argument('-is', '--image_size', metavar='WWWxHHH', help="the size of the resulting picture",
                        default=(2352, 1568), action=ResolutionAction)
    parser.add_argument('-ps', '--preview_size', metavar='WWWxHHH', help="the size of the preview image",
                        default=(900, 600), action=ResolutionAction)
    parser.add_argument('-pt', '--pose_time', metavar='<int>', type=int, help="countdown time", default=3)
    parser.add_argument('-st', '--slideshow_time', metavar='<int>', type=int,
                        help="Display time of pictures in the slideshow", default=5)
    parser.add_argument('-dt', '--display_time', metavar='<int>', type=int, help="Display time for assembled picture",
                        default=10)
    parser.add_argument('--theme', metavar='<string>', type=str, help="display theme name", default="default")
    parser.add_argument('--cam', metavar='list of <string>', type=str, help="cameras to use", nargs="+",
                        default=['sony_wifi', 'picam', 'webcam', 'dslr', 'dummicam'])
    parser.add_argument('--printer', metavar='<NAME>', help="Name of CUPS printer", default="Canon_SELPHY_CP1300")
    parser.add_argument('--ssid', metavar='<NAME>', help="SSID for sony wifi", default="DIRECT-LKE0:ILCE-6000")
